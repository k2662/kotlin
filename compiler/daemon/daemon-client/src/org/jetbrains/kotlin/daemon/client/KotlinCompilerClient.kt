...
object KotlinCompilerClient {
    ...
    fun connectAndLease(
        ...
        val ignoredDaemons = mutableSetOf<File>()
        return connectLoop(reportingTargets, autostart) { isLastAttempt ->
            ...
            when (result) {
                is DaemonSearchResult.Found -> result.compileService.leaseImpl().also {
                    // the null value here means that the daemon is already dying,
                    // so we shall query other daemons or start a new one
                    if (it == null) ignoredDaemons.add(result.runFileMarker)
                }
    ...
    private fun tryFindSuitableDaemonOrNewOpts(
        ...
        val aliveWithMetadata = try {
            walkDaemons(registryDir, compilerId, timestampMarker, report = report, filter = { file, _ -> file !in ignoredDaemons }).toList()
        } finally {
