    ...
    override fun processAfterAllModules(someAssertionWasFailed: Boolean) {
        ...
        if (!testReport.isEmpty()) {
            ...
            if (isDifferenceExplained && txtDiffPath.exists()) {
                assertions.fail {
                    "No ABI difference found. Remove ${CodegenTestDirectives.JVM_ABI_K1_K2_DIFF.name} directive and $txtDiffPath"
                }
            }
            if (isDifferenceExplained) {
            ...
            if (txtDiffPath.exists()) {
                assertions.fail { "No ABI difference found. Remove $txtDiffPath" }
            }
        }
    }
