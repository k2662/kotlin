...
class UnknownVersionRequirementTest {
    ...
    fun incorrectVersionRequirementHandledAsUnknown() {
        ...
        val incorrect = Metadata(KotlinClassMetadata.CLASS_KIND, KotlinClassMetadata.COMPATIBLE_METADATA_VERSION, d1, d2, extraInt = 0)

        val withInvalidRequirement = incorrect.readAsKmClass()
        assertEquals(2, withInvalidRequirement.versionRequirements.size)
        assertEquals(
            "[$requireKotlinValue, KmVersionRequirement(kind=UNKNOWN, level=HIDDEN, version=256.256.256, errorCode=null, message=null)]",
            withInvalidRequirement.versionRequirements.toString()
        )

        val rewritten = KotlinClassMetadata.writeClass(withInvalidRequirement).readAsKmClass()
